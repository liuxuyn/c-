...
void out_text16(int x,int y,char s[],int color)
{
    ...
    register i,j,k;
    unsigned char qh,wh;      //记录汉字库地址（区号，位号）
...
*****函数名：void word_write                                    *
*****功能：输出12或16大小的汉字                                  *
*****参数说明：(x,y)坐标，word输出字串，color字体颜色,charsize大小*
*****************************************************************/
...
*****函数名：void draw_bt                                       *
*****功能：画按钮显示按钮名                                      *
*****参数说明：*bt按钮指针，color字体颜色，charsize字体大小       *
*****************************************************************/
void draw_bt(BT *bt,int color,int charsize)     
{
...
/****************************************************************
*****函数名：void draw_btlight                                  *
*****功能：画按钮效果                                            *
*****参数说明：*bt按钮指针，state为状态标志位                    *
*****************************************************************/
...
/****************************************************************
*****函数名：void draw_roadin_s                                 *
*****功能：画十字车辆初始化界面时的路面函数                       *
*****参数说明：左上角（x0，y0），长len，宽wid，路宽rwid          *
*****************************************************************/
void draw_roadin_s(int x0,int y0,int len,int wid,int rwid)
{
    ...
    poly[0]=midx-rwid/2;        poly[1]=y0;          	   
    poly[2]=poly[0];            poly[3]=y0+(y1-y0-rwid)/2;   	  
    poly[4]=x0;                 poly[5]=poly[3];                      	  	  
    poly[6]=x0;                 poly[7]=poly[5]+rwid;	  
    poly[8]=poly[0];            poly[9]=poly[7];	  
    poly[10]=poly[0];           poly[11]=y1;	  
    poly[12]=midx+rwid/2;       poly[13]=y1;	  
    poly[14]=poly[12];          poly[15]=y1-(y1-y0-rwid)/2;	  
    poly[16]=x1;                poly[17]=poly[15];	  
    poly[18]=x1;                poly[19]=poly[17]-rwid;	  
    poly[20]=poly[14];          poly[21]=poly[19];           
    poly[22]=poly[20];          poly[23]=y0;	  
    poly[24]=poly[0];           poly[25]=poly[1];	  
...
void draw_roadin_d(int x0,int y0,int len,int wid,int rwid)
{
    ...
    poly[0]=x0;             poly[1]=y0+(y1-y0-rwid)/2;                       	  	  
    poly[2]=x0;             poly[3]=poly[1]+rwid;
    poly[4]=midx-rwid/2;    poly[5]=poly[3];
    poly[6]=poly[4];        poly[7]=y1;
    poly[8]=midx+rwid/2;    poly[9]=y1;
    poly[10]=poly[8];       poly[11]=y1-(y1-y0-rwid)/2;
    poly[12]=x1;            poly[13]=poly[11];
    poly[14]=x1;            poly[15]=poly[13]-rwid;	  
...
*****函数名：void draw_box                                      *
*****功能：画输入框的函数                                        *
*****参数说明：box为BOX结构指针，color为填充颜色                 *
*****************************************************************/
void draw_box(BOX *box,int color) 
{
    int x0=box->bx0;
    int y0=box->by0;
    int x1=x0+box->blen;
...
*****函数名：void draw_gg_s                                     *
*****功能：画十字路口路的四周                                    *
*****参数说明：count为draw_time需的计数，tm为TM结构指针，        *
*****         g为draw_4g需的时间存储数组指针                     *
*****************************************************************/
...
/****************************************************************
*****函数名：void draw_road_s                                   *
*****功能：画仿真时十字路面的函数                                *
*****参数说明：void                                             *
*****************************************************************/
void draw_road_s(void)		 
{
    int x0=BXO,y0=BYO;
    int ie,iw,is,in;
    int bk[32];
    ...
    //画路口停车线
    line(x0+3*RY+RR,y0-3*RX,x0+3*RY+RR,y0);        
...
/****************************************************************
*****函数名：void draw_road_d                                   *
*****功能：画仿真时丁字路面的函数                                *
*****参数说明：void                                             *
*****************************************************************/
void draw_road_d(void)		 
{
    int x0=BXO1,y0=BYO1;
    int ie,iw,is,in;
    int bk[20];
    ...
    //画路口停车线
    line(x0+2*RY+RR,y0-2*RX,x0+2*RY+RR,y0);         
...
/****************************************************************
*****函数名：void lamp_frame_s                                  *
*****功能：画仿真时十字路路灯框架的函数                          *
*****参数说明：（x,y）坐标，flag为路，color颜色，w、h宽与高      *
*****注：被void draw_lamp_s调用                                 *
*****************************************************************/
...
/****************************************************************
*****函数名：void lamp_frame_d                                  *
*****功能：画仿真时丁字路路灯框架的函数                          *
*****参数说明：（x,y）坐标，flag为路，color颜色，w、h宽与高      *
*****注：被void draw_lamp_d调用                                 *
*****************************************************************/
...
*****函数名：void lamp_arrow                                      *
*****功能：画交通灯箭头的函数（共用）                              *
*****参数说明：（x,y）箭头中心坐标，flag为路，color颜色，rate放大比 *
*****注：被void draw_lamp_s/d调用                                 *
*******************************************************************/
...
void draw_lamp_s(struct LAMP * pl)
{
    ...
    switch(f)
    {
         ...
         //1表示指向东，东南西北依此类推
         case 1:		//画东路交通灯
                lamp_arrow(x0-h/2,y0-5*w/2,4,c1,rate);	
