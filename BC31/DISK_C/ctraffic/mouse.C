...
/*检测鼠标是否在窗口上的函数*/
int mouse_onwin(int sx,int sy,int ex,int ey)
{
...
/*检测鼠标是否在输入框上*/
int mouse_onbox(BOX *box)
{
    if(MS.mx>(box->bx0)&&MS.mx<(box->bx0+box->blen)&&MS.my>(box->by0)
                        &&MS.my<(box->by0+box->bwid))
        return 1;	  
    else 
        return 0;
}