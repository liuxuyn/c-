...
{
    int wordw=8,wordh=8;
    line_new(*cursorx,local[1]+5*wordh+1,*cursorx,local[1]+8*wordh-1,BLACK,0);
}
...
// 1保存成功；2保存失败；3剪切提示；4缩放失败提示；5打开失败提示
{
    char transport[50]=".//picture//";
    char word;                              //输入键值
    char input[50];
    int wordh=textheight("w"),wordw=textwidth("w");
    int numofwordw=640/wordw,numofwordh=480/wordh;
    int local[4];
    ...
    while(1)
    {
        ...
        if(func_No==11||func_No==12||func_No==241||fun_No==242) drawcursor(&cursorx,local);

        if(mouse_press(local[0]+10*textw,local[1]+9*texth,local[0]+14*textw,local[1]+11*texth)==1)//点击取消按钮
        {
            exitdialog(storage);
            delay(50);
            return 0;
        }
        if(mouse_press(local[0]+2*textw,local[1]+9*texth,local[0]+6*textw,local[1]+11*texth)==1)//点击确定
        {
